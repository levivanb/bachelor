****************************************************
* ROLLING REGRESSION, JUN19 - JAN20
****************************************************
clear all
set more off
...
* Restrict sample
keep if inrange(date, td(01jun2019), td(1jan2020))
...
* Keep top 25 most negative
keep in 1/25

* Keep only identifiers needed for FEARS construction
keep tic query

gen str20 block = "jun19-jan20"

* Optional: enforce uniqueness (should already hold)
duplicates drop

save "$data_int/term_betas_25TERMS_jun2019_jan2020.dta", replace
